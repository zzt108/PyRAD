...
class myFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.Bind(wx.EVT_LISTBOX_DCLICK, self.on_dclick, self.lb_test)
        self.tc_test = wx.TextCtrl(self.panel, wx.ID_ANY, "", pos=(60,30))
    def on_test(self, event):
        print("Button pressed")
        event.Skip()
    def on_dclick(self, event):
        listno = self.lb_test.GetSelection()
        print(self.lb_test.GetString(listno)+" lives in Valhalla")
        event.Skip()    
class myApp(wx.App):
    def OnInit(self):
